...
public class BpelSerializer implements ElementVisitor {
	...
	private Element traversePathUntil(ControlOutputPort predecessor, Node until) throws BpelSerializerException {
		...
		Element sequenceElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "sequence");
			...
			if (currentNode instanceof BpelAtomicActivity) {
				...
				if (andSplit.getCorrespondingAndJoin() != null) {
					...
					if (syncingNode.getOutgoingTransitions().size() == 1 && syncingNode.getOutgoingTransitions().get(0).getDestinationNode() == andJoin) {
						...
						Element linksElement = document.createElementNS(BPEL_NAMESPACE_URI, "links");
						Element linkElementA = document.createElementNS(BPEL_NAMESPACE_URI, "link");
						...
						Element linkElementB = document.createElementNS(BPEL_NAMESPACE_URI, "link");
						...
						Element sourcesElement = document.createElementNS(BPEL_NAMESPACE_URI, "sources");
						Element sourceElement = document.createElementNS(BPEL_NAMESPACE_URI, "source");
						...
						Element targetsElement = document.createElementNS(BPEL_NAMESPACE_URI, "targets");
						Element targetElement = document.createElementNS(BPEL_NAMESPACE_URI, "target");
						...
						sourcesElement = document.createElementNS(BPEL_NAMESPACE_URI, "sources");
						sourceElement = document.createElementNS(BPEL_NAMESPACE_URI, "source");
						...
						targetsElement = document.createElementNS(BPEL_NAMESPACE_URI, "targets");
						targetElement = document.createElementNS(BPEL_NAMESPACE_URI, "target");
...
//						log.error("The WS-BPEL serializer cannot serialize syncs between branches (yet)!");
	...
	public void visit(BpelAndSplit andSplit) {
		...
		Element flowElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "flow");
	...
	public void visit(BpelAssignActivity assignActivity) {
		...
		Element assignElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "assign");
	...
	public void visit(BpelCopy copy) {
		...
		Element copyElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "copy");
		Element oldCurrentElement = this.currentElement;
	...
	public void visit(BpelCopyExpressionToExpressionActivity copyExpressionToExpressionActivity) {
		...
		Element fromElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "from");
		...
		if (source instanceof XpathExpressionImpl) {
			...
			for (Map.Entry<String, String> namespace : xpathSource.getNamespaces().entrySet()) {
				fromElement.setAttribute("xmlns:" + namespace.getKey(), namespace.getValue());
			}
		} else {
		...
		Element toElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "to");
	...
	public void visit(BpelFromVariable from) {
		...
		if (query != null) {
			Element queryElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "query");
			...
			for (Map.Entry<String, String> entry : from.getNamespaceDeclarations().entrySet()) {
				queryElement.setAttribute("xmlns:" + entry.getKey(), entry.getValue());
			}
...
//		fromElement.setTextContent("$" + getValueFromNestedDictionaryActivity.getSourceVariableName() + "/nd:firstKey[@key=" + getValueFromNestedDictionaryActivity.getFirstKey() + "]");
	...
	public void visit(BpelImport bpelImport) {
		...
		Element importElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "import");
	...
	public void visit(BpelInitializeDictionaryVariableActivity initializeDictionaryVariableActivity) {
		...
		Element copyElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "copy");
		Element fromElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "from");
		Element literalElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "literal");
	...
	public void visit(BpelInvokeMonitoringServiceActivity invokeActivity) {
		...
		importElement =  this.document.createElementNS(BPEL_NAMESPACE_URI, "import");
		importElement.setAttribute("importType", "http://schemas.xmlsoap.org/wsdl/");
		...
		Element partnerLinkElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "partnerLink");
		...
		Element inputVariableElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "variable");
		...
		Element outputVariableElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "variable");
		...
		copyElement.appendChild(toElement);
		assignElement.appendChild(copyElement);
		copyElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "copy");
		fromElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "from");
		...
		copyElement.appendChild(fromElement);
		toElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "to");
		toElement.setAttribute("variable", "backEndMonitorInput");
		toElement.setAttribute("part", "parameters");
		Element queryElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "query");
		...
		Element invokeElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "invoke");
	...
	public void visit(BpelEmptyActivity emptyActivity) {
		...
		Element emptyElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "empty");
	...
	public void visit(BpelReceiveActivity receiveActivity) {
		...
		Element receiveElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "receive");
	...
	public void visit(BpelReplyActivity replyActivity) {
		...
		Element replyElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "reply");
	...
	public void visit(BpelScopeActivity scopeActivity) {
		...
		Element scopeElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "scope");
		...
		Element partnerLinksElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "partnerLinks");
		...
		Element variablesElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "variables");
	...
	public void visit(BpelThrowActivity throwActivity) {
		...
		Element throwElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "throw");
	...
	public void visit(BpelVariableElement variable) {
		...
		Element variableElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "variable");
	...
	public void visit(BpelXorJoin xorJoin) {
		...
		Element repeatUntilElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "repeatUntil");
		...
		Element conditionElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "condition");
	...
	public void visit(BpelXorSplit xorSplit) {
		...
		while (iterator.hasNext()) {
			...
			if (condition != null) {
				Element elseifElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "elseif");
				conditionElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "condition");
				if (condition instanceof XpathExpressionImpl) {
					XpathExpressionImpl xpathCondition = (XpathExpressionImpl) condition;
					conditionElement.setTextContent(xpathCondition.getExpression());
					for (Map.Entry<String, String> namespace : xpathCondition.getNamespaces().entrySet()) {
				...
				Element elseElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "else");
				childElement = traversePathUntil(controlOutputPort, xorSplit.getCorrespondingXorJoin());
	...
	public void visit(BpelProcess process) {
		...
		Element processElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "process");
		...
		/////*************Added correlation set 
			Element corrSetsElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "correlationSets");
	...
	public void visit(BpelCompositeReceiveActivity receiveActivity) {
		...
		Element correlationsElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "correlations");
	...
	public void visit(BpelCorrelationSet corrSet) {
		...
		Element corrSetElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "correlationSet");
	...
	public void visit(BpelCorrelation bpelCorrelation) {
		...
		Element corrElement = this.document.createElementNS(BPEL_NAMESPACE_URI, "correlation");
