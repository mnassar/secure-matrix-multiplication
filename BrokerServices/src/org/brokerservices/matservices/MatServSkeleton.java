...
public class MatServSkeleton {
			...
			)
	{
		...
		if(add.getAdd_list().length == 2) //add files on broker
		{
			new Thread(new Runnable() {
				public void run() { addOnBroker(add_request);}}).start();
			return;
		}
		...
		new Thread(new Runnable() {
			public void run() {
				...
				try {
					...
					final String userToken = matA.getUser_token();
					...
					try {
						...
						String server_ip1 = add_request.getAdd_list()[0].getCloudURL();//loc1.getIP();
						...
						ugi.doAs(new PrivilegedExceptionAction<Void>() {
							public Void run() throws Exception {
								...
								//	System.out.println("URL: "+ add_request.getAdd_list()[1].getCloudURL());
								//	Location loc2 = new Location(add_request.getAdd_list()[1].getCloudURL());
								String server_ip2 = add_request.getAdd_list()[1].getCloudURL();//loc2.getIP();
								...
								String server_ip3 = add_request.getAdd_list()[2].getCloudURL();//loc3.getIP();

								Configuration conf4 = new Configuration();
								//Location loc4 = new Location(add_request.getAdd_list()[3].getCloudURL());
								String server_ip4 = add_request.getAdd_list()[3].getCloudURL();//loc4.getIP();
								...
								String pathA = "/"+add_request.getAdd_list()[0].getSplitName()+"/part-00000";
								if(!dfs.exists(new Path(dfs.getHomeDirectory()+pathA)))
									pathA = "/"+add_request.getAdd_list()[0].getSplitName()+"/part-m-00000";
								String pathB = "/"+add_request.getAdd_list()[1].getSplitName()+"/part-00000";
								if(!dfs2.exists(new Path(dfs2.getHomeDirectory()+pathB)))
									pathB = "/"+add_request.getAdd_list()[1].getSplitName()+"/part-m-00000";

								String pathC = "/"+add_request.getAdd_list()[2].getSplitName()+"/part-00000";
								if(!dfs3.exists(new Path(dfs3.getHomeDirectory()+pathC)))
									pathC = "/"+add_request.getAdd_list()[2].getSplitName()+"/part-m-00000";

								String pathD = "/"+add_request.getAdd_list()[3].getSplitName()+"/part-00000";
								if(!dfs4.exists(new Path(dfs4.getHomeDirectory()+pathD)))
									pathD = "/"+add_request.getAdd_list()[3].getSplitName()+"/part-m-00000";
								...
								try {
									...
									try {             
										...
										BrokerSOCResource result = new BrokerSOCResource(matA_ID+"_"+matB_ID);
										result.setFile_path(SOCConfiguration.BROKER_STORAGE_PATH+"/"+matA_ID+"_"+matB_ID);
										result.setUser_token(userToken);
										result.setResource_meta(new MatrixMeta(nRows, nCols, "decimal"));
						...
						logfile.write(" Time taken for addition in job number: "+ jobID+" with matrix size ("+nRows+" , "+ nCols+")" 
								+":  " + stopwatch.getTime() + " ms, in nanotime :"+(System.nanoTime()-start_time) +"\n");
						//fw2.close();
			...
			)
	{
		...
		new Thread(new Runnable() {
			public void run() {
				...
				if(!(comb1_found && comb2_found && comb3_found && comb4_found))  /// resplitting needed if any combination is not found
				{
					...
					try {
						...
						AdditiveSplitter splitter1 = new AdditiveSplitter(matA);
						boolean splitted = splitter1.Split(clouds.get(0),clouds.get(2), clouds.get(1), clouds.get(3));
						...
						AdditiveSplitter splitter2 = new AdditiveSplitter(matB);
						 splitted = splitter2.Split(clouds.get(0),clouds.get(3), clouds.get(1) , clouds.get(2));
				...
				logfile.write("Resplitting check is done for job "+ resplit_request.getJob_id());
				...
				resplit_callback.setJobID(resplit_request.getJob_id());
				...
				for(int j=0; j<4; j++)
				{
					splits[j] = new Split_type0();
					splits[j].setCloud_url("http://"+clouds.get(j).getIP()+":8080/CloudServices/services/MatServ");
				...
				try {
					resplitting_callback = new AdditiveSplittingServiceStub(resplit_request.getCallback());
					logfile.write("Sending resplitting callback : {jobID:"+resplit_callback.getJobID()+"} ,{op_id: "+resplit_callback.getOp_ID() +"} , {sub_jobID:"+resplit_callback.getSub_jobID() +"} , {cloud1:"+resplit_callback.getSplit()[0].getCloud_url() +"},{cloud2:"+resplit_callback.getSplit()[1].getCloud_url() +"},{cloud3:"+resplit_callback.getSplit()[2].getCloud_url() +"},{cloud4:"+resplit_callback.getSplit()[3].getCloud_url() +"}");
				...
				stopwatch.stop();
				logfile.write(" Time taken for resplitting in job number: "+ resplit_request.getJob_id() +" with matrix size ("+((MatrixMeta)matA.getResource_meta()).getnRows()+" , "+ ((MatrixMeta)matA.getResource_meta()).getnColumns()+")" 
						+":  " + stopwatch.getTime() + " ms,  in nanotime :"+(System.nanoTime()-start_time) +"\n");
...
//				Thread.currentThread().stop();
	...
	public void addOnBroker(org.brokerservices.www.matserv.Add add) {
		// TODO Auto-generated method stub
		File f1 = new File(SOCConfiguration.BROKER_STORAGE_PATH+"/"+add.getAdd_list()[0].getSplitName());
		File f2 = new File(SOCConfiguration.BROKER_STORAGE_PATH+"/"+add.getAdd_list()[1].getSplitName());
		...
		try {
			...
			matA = gson.fromJson(conn.getSOCResource(add.getMatA_ID()),BrokerSOCResource.class);
			matB = gson.fromJson(conn.getSOCResource(add.getMatB_ID()),BrokerSOCResource.class);
			...
			String result_filename = matA.getResource_id()+"_"+matB.getResource_id();


			//Add the two matrices
			File output = new File(SOCConfiguration.BROKER_STORAGE_PATH+"/"+result_filename);
			...
			{
				String sCurrentLine1 = new String(br1.readLine());
				StringTokenizer tokenzier1 = new StringTokenizer(sCurrentLine1, ",");

				String sCurrentLine2 = new String(br2.readLine());
				StringTokenizer tokenzier2 = new StringTokenizer(sCurrentLine2, ",");

				for (int j=0; j< nCols; j++)
				{
					Double val1 = Double.parseDouble(tokenzier1.nextToken());
					Double val2 = Double.parseDouble(tokenzier2.nextToken());
					Double value = val1 + val2;
					pw.print(value);

					if(j== nCols-1){ pw.println(); break;}
					pw.print(",");
			...
			BrokerSOCResource result = new BrokerSOCResource(result_filename);
			result.setFile_path(SOCConfiguration.BROKER_STORAGE_PATH+"/"+result_filename);
			result.setUser_token(matA.getUser_token());
			result.setResource_meta(new MatrixMeta(nRows, nCols, "decimal"));
